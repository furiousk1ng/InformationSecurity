...
class ConnectionTester(Tester):
    def do_test(self):
            ...
            result = ""
            if response == False:
                self.updater("Данный компьютер не подключен к интернету")
            else:
                self.updater("Данный компьютер подключен к интернету")
            return result
...
class WindowsUpdateTester(Tester):
    ...
    def do_test(self):
        try:
            ...
            if error:
                error_message = f"Ошибка: {error.decode('utf-8')}"
                self.updater(error_message)
                formatted_error = error_message.replace('\r\n', '\n')  # Замена Windows стиля новой строки
                self.updater(formatted_error)
            else:
                result_text = str(result, 'utf-8')
                self.updater(result_text)
                formatted_result = result_text.replace('\r\n', '\n')  # Замена Windows стиля новой строки
                self.updater( formatted_result)
        except Exception as e:
...
class Window(Tk):
    ...
    def __init__(self):
        ...
        Label(top_frame, text="Проверка антивирусного ПО", **label_style).pack(**label_pack)
